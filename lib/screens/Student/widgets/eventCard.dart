...
class EventCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      hasBudget =
          event.estimatedBudget != null && event.estimatedBudget!.isNotEmpty;
    } catch (e) {
  ...
  children: [
    ...
    ClipRRect(
      ...
      child: Stack(
        children: [
          ...
          // Improved overlay gradient - more depth and contrast
          Container(
            width: double.infinity,
            height: size.height * 0.25,
            decoration: BoxDecoration(
          ...
          // FIXED: Subtle pattern overlay with proper blend mode implementation
          Container(
            width: double.infinity,
            height: size.height * 0.25,
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(16),
              color: Colors.black.withOpacity(0.05), // Required for blend mode
              backgroundBlendMode: BlendMode.overlay,
              image: const DecorationImage(
                image: NetworkImage(
                  'https://www.transparenttextures.com/patterns/cubes.png'),
                repeat: ImageRepeat.repeat,
                opacity: 0.05, // Use opacity here instead
              ),
            ),
          ),
        ],
    ...
    // Enhanced Content overlay with better spacing and typography
    Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Enhanced date and time row with better visual indicators
          Container(
          ...
          // Location row with enhanced type indicator
          Row(
            children: [
              Container(
                padding: const EdgeInsets.all(6),
              ...
              // Enhanced location type chip with animation
                ...
                TweenAnimationBuilder<double>(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: _getLocationTypeColor(event).withOpacity(0.5),
                          blurRadius: 4,
          ...
          // Footer row with enhanced visual design
          Container(
            ...
            child: _buildFooterRow(event),
          ),
        ],
      ),
    ),
    
    // Enhanced multi-day indicator badge with animation
    if (_isMultidayEvent(event))
      Positioned(
        ...
        child: Material(
          ...
          child: InkWell(
            onTap: () {
              // Optional: Show more details about the multi-day schedule
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('${event.title} runs for ${_getEventDuration(event)} days'),
    ...
    // Enhanced budget chat button with hover effect
    if (hasBudget)
      Positioned(
        right: 16,
        bottom: 16,
    ...
    // Interactive element to indicate the card is clickable
    // Positioned.fill(
    //   child: Material(
    //     color: Colors.transparent,
    //     child: InkWell(
    //       borderRadius: BorderRadius.circular(16),
    //       onTap: () {
    //         // Navigate to event details page or show a dialog with more info
    //         ScaffoldMessenger.of(context).showSnackBar(
    //           SnackBar(
  ...
  String getEventImageUrl(Event event) {
    ...
    if (locationType == 'outside college' ||
        eventType.contains('excursion') ||
        eventType.contains('trip')) {
      return "https://images.unsplash.com/photo-1507608616759-54f48f0af0ee?q=80&w=1740&auto=format&fit=crop";
    } else if (eventType.contains('workshop') ||
        eventType.contains('seminar')) {
      return "https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=1740&auto=format&fit=crop";
        ...
        eventType.contains('fest') ||
        eventType.contains('celebration')) {
      return "https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?q=80&w=1740&auto=format&fit=crop";
    } else if (eventType.contains('tech') ||
        eventType.contains('hackathon') ||
...
class BudgetDiscussionPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BudgetDiscussionPage> createState() =>
      _BudgetDiscussionPageState();
}
...
class _BudgetDiscussionPageState extends ConsumerState<BudgetDiscussionPage> {
  ...
  @override
  Widget build(BuildContext context) {
    final messageStream = ref.watch(discussionMessagesProvider(widget.eventId));

    // For first load with messages, scroll to bottom
    if (_isFirstLoad &&
        messageStream.maybeWhen(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text('Ask CMSai about Budget'),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            ...
            _buildGeminiQuestionButton(
                context, 'How can we stay within budget?'),
            ...
            _buildGeminiQuestionButton(
                context, 'Help allocate budget to categories'),
  ...
  Future<void> _sendUserMessageAndGetGeminiResponse(String question) async {
    ...
    try {
      ...
      final expenseSummary = categoryTotals.entries
...
""";
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('CMS response failed: $e')),
        );
      }
    } finally {
      if (mounted) {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: const [
            ...
            Text('• Ask CMS for AI-powered budget advice'),
            Text('• Discuss expense priorities and allocations'),
            Text('• Get real-time spending analysis'),
            SizedBox(height: 12),
            Text(
              'Tip: Click the CMS icon to get AI help with budget planning, cost reduction strategies, and financial advice.',
...
final discussionMessagesProvider =
    StreamProvider.family<List<DiscussionMessage>, String>(
  (ref, eventId) {
